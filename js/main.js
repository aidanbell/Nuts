...
const squirrelJobRender = (squirrel, nut = false) => {
  ...
  removeIcon.addEventListener('click', () =>  removeSquirrel(squirrel))
...
function storyCheck() {
  ...
  if (POP.thinkers.length && reportIdx === 4) {
    playNextReport();
    unlocker();
    THRESHOLD++;
  }
...
function unlocker() {
  switch(THRESHOLD) {
    case 1:
      ...
      break;
    case 3:
      let tab2 = document.getElementById('jobsites')
      tab2.classList.replace('locked', 'unlocked')
      tab2.textContent = 'Jobsites'
      break;
  }
}

function costChecker(cost, id, type) {
  if (type === 'nuts') {
  }
  if (type === 'thoughts') {
    if (nutsTotal >= cost) {
